...
| 7 | CLI (init, validate, freeze, run, report) | SHIPPED | F→G | five commands shipped: `init` (§F4), `validate` (§F1), `freeze` (§F2), `run`, and `report`; scaffold templates (§F3); README quickstart (§F5); `bakeoff init` now works into a directory that does not exist (§G1), and the stranger's path is pinned by `tests/test_quickstart_path.py` (§G3/§G4) |
| 8 | Bundled stub OpenAI-compatible server | SHIPPED | A |
| 9 | Deploy-grade packaging (pyproject, CI, quickstart, ledger) | SHIPPED | F→G | pyproject, CI, scrub-check, verify.sh, README shipped; JSONL ledger shipped in C and wired to `bakeoff run` in F0; the `bakeoff` entry point shipped in F0; the README quickstart is §F5 and the README-quickstart lint §A6 deferred is §F6; `scripts/live-check.sh` and the README's real-endpoint section shipped in G |
