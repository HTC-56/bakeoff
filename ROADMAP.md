...
| 7 | CLI (init, validate, freeze, run, report) | PARTIAL | F→G | five commands shipped: `init` (§F4), `validate` (§F1), `freeze` (§F2), `run`, and `report`; scaffold templates (§F3); README quickstart (§F5). Demoted 2026-08-29: `bakeoff init <dir>` — the README's first command — raises `FileNotFoundError` when the directory does not exist; every test wrote into an existing `tmp_path`, so no gate caught it. §G1 fixes it, §G3/§G4 pin the stranger's path |
| 8 | Bundled stub OpenAI-compatible server | SHIPPED | A |
| 9 | Deploy-grade packaging (pyproject, CI, quickstart, ledger) | PARTIAL | F→G | pyproject, CI, scrub-check, verify.sh, README shipped; JSONL ledger shipped in C and wired to `bakeoff run` in F0; the `bakeoff` entry point shipped in F0; the README quickstart is §F5 and the README-quickstart lint §A6 deferred is §F6. Demoted 2026-08-29: `scripts/live-check.sh` is named by SPEC.md (three times), locked by DECISIONS.md and cited by README.md, but was never written (§G5); the README lacks SPEC feature 9's "then a real endpoint" half (§G6) |
| — | docs/PROCESS.md (the loop story) | SHIPPED | F | written in §F7 |
...
- The report is rendered from `results.json` alone and never re-dates it, so `bakeoff report`
  ...
  phase (§E5).
- The `bakeoff` exit-code contract is 0 worked / 1 the bar was missed / 2 the audition is
  misconfigured (§F0).
- `init` writes no lockfile because freezing is a deliberate separate step (§F3).
- The README hero screenshot is the one open item and is human-gated.
