...
2. Gate: `uv run ruff check .` clean AND `uv run ruff format --check .` clean AND
   `uv run mypy` clean AND `uv run pytest` green AND
...
3. If green: mark the task "- [x]" in TODO.md, run `git status --short`, stage every
   SOURCE file you created or changed this session (new files, configs, uv.lock),
   commit with a message naming the task. Never stage generated output
   (.venv/, __pycache__/, .pytest_cache/, .mypy_cache/, .ruff_cache/, results.json,
   report HTML, logs, ledger.jsonl).
...
- Every HTTP call goes through the one client module; tests and the quickstart use
  the bundled stub server. **No network calls in tests, ever.**
- mypy stays `--strict`; never weaken strictness, add `type: ignore`, or loosen
  ruff rules to make a task pass. If strictness blocks the task, BLOCKED.md.
- Dependency surface stays exactly: httpx, pydantic, PyYAML, jsonschema, click.
  Add anything else ONLY if your task names it.
- The report is one self-contained HTML file with zero external requests. No
  frameworks, no CDN, no web fonts — anywhere in the repo.
